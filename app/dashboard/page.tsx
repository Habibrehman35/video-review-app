...
export default async function DashboardPage() {
  try {
    ...
    if (err?.message?.includes('NEXT_REDIRECT') || err?.digest?.includes('NEXT_REDIRECT') || err?.message === 'NEXT_REDIRECT') {
      throw err
    }
    console.error('DASHBOARD CRASH ERROR:', err)
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-slate-900 border border-red-500/30 p-8 rounded-3xl space-y-4 shadow-2xl">
          ...
          <h1 className="text-xl font-extrabold text-white">Asal Error Mil Gaya</h1>
          <p className="text-xs text-slate-400">Dashboard load hotay waqt yeh database ya code error aa raha hai:</p>
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl text-xs font-mono text-red-400 overflow-x-auto whitespace-pre-wrap">
            {err?.message || JSON.stringify(err, null, 2)}
          </div>
          <p className="text-xs text-slate-300">Yeh error message mujhe yahan chat mein bhejiye, main foran isay theek kar deta hoon!</p>
        </div>
      </div>
    )
  }
